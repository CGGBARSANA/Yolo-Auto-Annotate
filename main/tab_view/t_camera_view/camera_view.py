...
from main.utils import create_save_directory, save_current_annotation, save_yolo_format
from main.tab_view.t_camera_view import detect_cameras_safe, populate_camera_combo
...
class CameraAnnotation(QWidget):
    ...
    def detect_objects(self, frame, camera_id):
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                if hasattr(result, 'boxes') and result.boxes is not None and len(result.boxes) > 0:
                    ...
                    for i, (box, conf, cls_id) in enumerate(zip(boxes, confidences, class_ids)):
                        ...
                        # Draw manage_label_btn with confidence
                        label = f"{class_name}: {conf:.2f}"
                        label_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)[0]

                        # Draw manage_label_btn background
                        cv2.rectangle(annotated_frame,
                                      (x1, y1 - label_size[1] - 10),
                                      (x1 + label_size[0], y1),
                                      (0, 255, 0), -1)

                        # Draw manage_label_btn text
            ...
            print(f"Detection error:{detections} {e}")
            return frame, []
    ...
    def update_frame(self, cam_id):
        ...
        try:
            ...
            try:
                ...
                # Scale to fit manage_label_btn while maintaining aspect ratio
