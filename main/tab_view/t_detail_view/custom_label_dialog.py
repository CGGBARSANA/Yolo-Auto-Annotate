...
class CustomLabelDialog(QDialog):
    ...
    def add_label(self):
        """Add a new custom manage_label_btn"""
        text, ok = QInputDialog.getText(self, 'Add Custom Label', 'Enter manage_label_btn name:')
        if ok and text.strip():
            ...
            try:
                ...
                QMessageBox.information(self, "Success",
                                        f"Added manage_label_btn '{label_name}' with ID {class_id}")
                self.refresh_list()
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Error adding manage_label_btn: {e}")

    def edit_label(self):
        """Edit selected custom manage_label_btn"""
        current_item = self.list_widget.currentItem()
        if not current_item or current_item.text() == "No custom labels defined":
            QMessageBox.warning(self, "No Selection", "Please select a manage_label_btn to edit.")
            return

        # Extract manage_label_btn name from the list item text
        ...
        text, ok = QInputDialog.getText(self, 'Edit Custom Label',
                                        'Enter new manage_label_btn name:', text=old_name)
        if ok and text.strip():
            new_name = text.strip()
            ...
            try:
                if self.label_manager.edit_custom_label(old_name, new_name):
                    ...
                    QMessageBox.warning(self, "Error", "Failed to edit manage_label_btn.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Error editing manage_label_btn: {e}")

    def remove_label(self):
        """Remove selected custom manage_label_btn"""
        current_item = self.list_widget.currentItem()
        if not current_item or current_item.text() == "No custom labels defined":
            QMessageBox.warning(self, "No Selection", "Please select a manage_label_btn to remove.")
            return
        ...
        if reply == QMessageBox.Yes:
            try:
                if self.label_manager.remove_custom_label(label_name):
                    QMessageBox.information(self, "Success",
                                            f"Removed manage_label_btn '{label_name}'")
                    self.refresh_list()
                else:
                    QMessageBox.warning(self, "Error", "Failed to remove manage_label_btn.")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Error removing manage_label_btn: {e}")