...
class CustomLabelManager:
    ...
    def add_custom_label(self, label_name):
        """Add a new custom manage_label_btn with consistent class ID based on current mode"""
    ...
    def remove_custom_label(self, label_name):
        """Remove a custom manage_label_btn"""
        removed = False
    ...
    def edit_custom_label(self, old_name, new_name):
        """Edit a custom manage_label_btn name"""
        if self.current_mode == "custom_only":
    ...
    def _reassign_custom_only_ids(self):
        """Reassign custom-only manage_label_btn IDs to be sequential starting from 0"""
    ...
    def get_class_id(self, label_name, model_class_names=None, class_selection_mode=None):
        """Get class ID for a manage_label_btn (model or custom) respecting the current mode"""
        ...
        if class_selection_mode == "model_only":
            ...
            if label_name in self.custom_only_labels:
                ...
                # Create new custom manage_label_btn starting from current count
            ...
            # If not found, add as new custom manage_label_btn starting after highest model ID
            class_id = self.next_custom_id
            self.custom_labels[label_name] = class_id
            self.next_custom_id += 1
            self.save_custom_labels()
    ...
    def get_custom_labels_list(self):
        """Get list of custom manage_label_btn names based on current mode"""
        if self.current_mode == "custom_only":
    ...
    def get_label_info_for_display(self):
        """Get formatted manage_label_btn information for display in dialogs"""
        info = []
